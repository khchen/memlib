...
################################################################################
# Modify the openssl.nim to use memlib,
# so that we can get a standalone EXE without any dependencies
################################################################################
...
when sslVersion != "":
  const
    ...
    DLLSSLData = staticReadDll DLLSSLName
    DLLUtilData = staticReadDll DLLUtilName

else:
  when defined(cpu64):
    const
      (DLLSSLName*, DLLSSLData) = staticReadDllWithName "(libssl-1_1-x64|ssleay64|libssl64).dll"
      (DLLUtilName*, DLLUtilData) = staticReadDllWithName "(libcrypto-1_1-x64|libeay64).dll"
  else:
    const
      (DLLSSLName*, DLLSSLData) = staticReadDllWithName "(libssl-1_1|ssleay32|libssl32).dll"
      (DLLUtilName*, DLLUtilData) = staticReadDllWithName "(libcrypto-1_1|libeay32).dll"

proc loadModule(): (MemoryModule, MemoryModule) =
  var
    sslMod {.global.}: MemoryModule
    utilMod {.global.}: MemoryModule

  # libssl needs libcrypto, so hook libcrypto first.
  if utilMod.isNil:
    utilMod = checkedLoadLib(DLLUtilData)
    utilMod.hook(DLLUtilName)

  if sslMod.isNil:
    sslMod = checkedLoadLib(DLLSSLData)
    sslMod.hook(DLLSSLName)

  result = (sslMod, utilMod)

let (sslMod, utilMod) = loadModule()
buildPragma { memlib: sslMod }: libssl
buildPragma { memlib: utilMod }: libutil
...
proc TLSv1_method*(): PSSL_METHOD{.cdecl, libssl, importc.}
...
when compileOption("dynlibOverride", "ssl") or defined(noOpenSSLHacks):
  ...
  when defined(openssl10):
    proc SSL_library_init*(): cint {.cdecl, libssl, importc, discardable.}
    proc SSL_load_error_strings*() {.cdecl, libssl, importc.}
    proc SSLv23_method*(): PSSL_METHOD {.cdecl, libssl, importc.}
    proc SSLeay(): culong {.cdecl, libutil, importc.}

    proc getOpenSSLVersion*(): culong =
      SSLeay()
  else:
    proc OPENSSL_init_ssl*(opts: uint64, settings: uint8): cint {.cdecl, libssl, importc, discardable.}
    proc SSL_library_init*(): cint {.discardable.} =
      ## Initialize SSL using OPENSSL_init_ssl for OpenSSL >= 1.1.0
      return OPENSSL_init_ssl(0.uint64, 0.uint8)

    proc TLS_method*(): PSSL_METHOD {.cdecl, libssl, importc.}
    proc SSLv23_method*(): PSSL_METHOD =
      TLS_method()

    proc OpenSSL_version_num(): culong {.cdecl, libutil, importc.}
  ...
  when defined(libressl) or defined(openssl10):
    proc SSL_state(ssl: SslPtr): cint {.cdecl, libssl, importc.}
    proc SSL_in_init*(ssl: SslPtr): cint {.inline.} =
      SSl_state(ssl) and SSL_ST_INIT
  else:
    proc SSL_in_init*(ssl: SslPtr): cint {.cdecl, libssl, importc.}
    proc SSL_CTX_set_ciphersuites*(ctx: SslCtx, str: cstring): cint {.cdecl, libssl, importc.}

  template OpenSSL_add_all_algorithms*() = discard

  proc SSLv23_client_method*(): PSSL_METHOD {.cdecl, libssl, importc.}
  proc SSLv2_method*(): PSSL_METHOD {.cdecl, libssl, importc.}
  proc SSLv3_method*(): PSSL_METHOD {.cdecl, libssl, importc.}
  ...
  proc sslModule(): MemoryModule {.inline.} =
    result = loadModule()[0]

  proc utilModule(): MemoryModule {.inline.} =
    result = loadModule()[1]

  proc symNullable(dll: MemoryModule, name: string, alternativeName = ""): pointer =
    # Load from DLL.
    if not dll.isNil:
  ...
  proc SSL_library_init*(): cint {.discardable.} =
    ...
    if not newInitSym.isNil:
      ...
      return 1 # this hack works, however, need to know why OPENSSL_init_ssl() fail.
...
proc ERR_load_BIO_strings*(){.cdecl, libutil, importc.}

proc SSL_new*(context: SslCtx): SslPtr{.cdecl, libssl, importc.}
proc SSL_free*(ssl: SslPtr){.cdecl, libssl, importc.}
proc SSL_get_SSL_CTX*(ssl: SslPtr): SslCtx {.cdecl, libssl, importc.}
proc SSL_set_SSL_CTX*(ssl: SslPtr, ctx: SslCtx): SslCtx {.cdecl, libssl, importc.}
proc SSL_CTX_set_session_id_context*(context: SslCtx, sid_ctx: string, sid_ctx_len: int){.cdecl, libssl, importc.}
proc SSL_get0_verified_chain*(ssl: SslPtr): PSTACK {.cdecl, libssl,
    importc.}
proc SSL_CTX_new*(meth: PSSL_METHOD): SslCtx{.cdecl,
    libssl, importc.}
proc SSL_CTX_load_verify_locations*(ctx: SslCtx, CAfile: cstring,
    CApath: cstring): cint{.cdecl, libssl, importc.}
proc SSL_CTX_free*(arg0: SslCtx){.cdecl, libssl, importc.}
proc SSL_CTX_set_verify*(s: SslCtx, mode: int, cb: proc (a: int, b: pointer): int {.cdecl.}){.cdecl, libssl, importc.}
proc SSL_get_verify_result*(ssl: SslPtr): int{.cdecl,
    libssl, importc.}

proc SSL_CTX_set_cipher_list*(s: SslCtx, ciphers: cstring): cint{.cdecl, libssl, importc.}
proc SSL_CTX_use_certificate_file*(ctx: SslCtx, filename: cstring, typ: cint): cint{.
    stdcall, libssl, importc.}
proc SSL_CTX_use_certificate_chain_file*(ctx: SslCtx, filename: cstring): cint{.
    stdcall, libssl, importc.}
proc SSL_CTX_use_PrivateKey_file*(ctx: SslCtx,
    filename: cstring, typ: cint): cint{.cdecl, libssl, importc.}
proc SSL_CTX_check_private_key*(ctx: SslCtx): cint{.cdecl, libssl,
    importc.}

proc SSL_CTX_get_ex_new_index*(argl: clong, argp: pointer, new_func: pointer, dup_func: pointer, free_func: pointer): cint {.cdecl, libssl, importc.}
proc SSL_CTX_set_ex_data*(ssl: SslCtx, idx: cint, arg: pointer): cint {.cdecl, libssl, importc.}
proc SSL_CTX_get_ex_data*(ssl: SslCtx, idx: cint): pointer {.cdecl, libssl, importc.}

proc SSL_set_fd*(ssl: SslPtr, fd: SocketHandle): cint{.cdecl, libssl, importc.}

proc SSL_shutdown*(ssl: SslPtr): cint{.cdecl, libssl, importc.}
proc SSL_set_shutdown*(ssl: SslPtr, mode: cint) {.cdecl, libssl, importc: "SSL_set_shutdown".}
proc SSL_get_shutdown*(ssl: SslPtr): cint {.cdecl, libssl, importc: "SSL_get_shutdown".}
proc SSL_connect*(ssl: SslPtr): cint{.cdecl, libssl, importc.}
proc SSL_read*(ssl: SslPtr, buf: pointer, num: int): cint{.cdecl, libssl, importc.}
proc SSL_write*(ssl: SslPtr, buf: cstring, num: int): cint{.cdecl, libssl, importc.}
proc SSL_get_error*(s: SslPtr, ret_code: cint): cint{.cdecl, libssl, importc.}
proc SSL_accept*(ssl: SslPtr): cint{.cdecl, libssl, importc.}
proc SSL_pending*(ssl: SslPtr): cint{.cdecl, libssl, importc.}
...
proc BIO_free*(b: BIO): cint{.cdecl, libutil, importc.}

proc ERR_print_errors_fp*(fp: File){.cdecl, libssl, importc.}

proc ERR_error_string*(e: culong, buf: cstring): cstring{.cdecl,
    libutil, importc.}
proc ERR_get_error*(): culong{.cdecl, libutil, importc.}
proc ERR_peek_last_error*(): culong{.cdecl, libutil, importc.}

proc OPENSSL_config*(configName: cstring){.cdecl, libssl, importc.}

proc OPENSSL_sk_num*(stack: PSTACK): int {.cdecl, libssl, importc.}
...
proc SSL_get_servername*(ssl: SslPtr, typ: cint = TLSEXT_NAMETYPE_host_name): cstring {.cdecl, libssl, importc.}
...
proc SSL_CTX_set_psk_client_callback*(ctx: SslCtx; callback: PskClientCallback) {.cdecl, libssl, importc.}
  ## Set callback called when OpenSSL needs PSK (for client).

proc SSL_CTX_set_psk_server_callback*(ctx: SslCtx; callback: PskServerCallback) {.cdecl, libssl, importc.}
  ## Set callback called when OpenSSL needs PSK (for server).

proc SSL_CTX_use_psk_identity_hint*(ctx: SslCtx; hint: cstring): cint {.cdecl, libssl, importc.}
  ## Set PSK identity hint to use.

proc SSL_get_psk_identity*(ssl: SslPtr): cstring {.cdecl, libssl, importc.}
  ## Get PSK identity.
...
proc bioNew*(b: PBIO_METHOD): BIO{.cdecl, libutil, importc: "BIO_new".}
proc bioFreeAll*(b: BIO){.cdecl, libutil, importc: "BIO_free_all".}
proc bioSMem*(): PBIO_METHOD{.cdecl, libutil, importc: "BIO_s_mem".}
proc bioCtrlPending*(b: BIO): cint{.cdecl, libutil, importc: "BIO_ctrl_pending".}
proc bioRead*(b: BIO, Buf: cstring, length: cint): cint{.cdecl,
    libutil, importc: "BIO_read".}
proc bioWrite*(b: BIO, Buf: cstring, length: cint): cint{.cdecl,
    libutil, importc: "BIO_write".}

proc sslSetConnectState*(s: SslPtr) {.cdecl,
    libssl, importc: "SSL_set_connect_state".}
proc sslSetAcceptState*(s: SslPtr) {.cdecl,
    libssl, importc: "SSL_set_accept_state".}

proc sslRead*(ssl: SslPtr, buf: cstring, num: cint): cint{.cdecl,
      libssl, importc: "SSL_read".}
proc sslPeek*(ssl: SslPtr, buf: cstring, num: cint): cint{.cdecl,
    libssl, importc: "SSL_peek".}
proc sslWrite*(ssl: SslPtr, buf: cstring, num: cint): cint{.cdecl,
    libssl, importc: "SSL_write".}

proc sslSetBio*(ssl: SslPtr, rbio, wbio: BIO) {.cdecl,
    libssl, importc: "SSL_set_bio".}

proc sslDoHandshake*(ssl: SslPtr): cint {.cdecl,
    libssl, importc: "SSL_do_handshake".}



proc ErrClearError*(){.cdecl, libutil, importc: "ERR_clear_error".}
proc ErrFreeStrings*(){.cdecl, libutil, importc: "ERR_free_strings".}
proc ErrRemoveState*(pid: cint){.cdecl, libutil, importc: "ERR_remove_state".}
...
proc RSA_verify*(kind: cint, origMsg: pointer, origMsgLen: cuint, signature: pointer,
    signatureLen: cuint, rsa: PRSA): cint {.cdecl, libssl, importc.}
...
proc RSA_public_decrypt*(flen: cint, fr: ptr cuchar, to: ptr cuchar, rsa: PRSA, padding: PaddingType): cint {.cdecl,
    libssl, importc.}
proc RSA_free*(rsa: PRSA) {.cdecl, libssl, importc.}
proc RSA_size*(rsa: PRSA): cint {.cdecl, libssl, importc.}
...
when not defined(nimDisableCertificateValidation) and not defined(windows):
  ...
  proc SSL_get_peer_certificate*(ssl: SslCtx): PX509{.cdecl, libssl,
      importc.}

  proc X509_get_subject_name*(a: PX509): PX509_NAME{.cdecl, libssl, importc.}

  proc X509_get_issuer_name*(a: PX509): PX509_NAME{.cdecl, libutil, importc.}
  ...
  proc X509_check_host*(cert: PX509, name: cstring, namelen: cint, flags:cuint, peername: cstring): cint {.cdecl, libssl, importc.}
